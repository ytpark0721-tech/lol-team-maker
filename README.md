...
- 소환사명 입력 시 op.gg에서 모스트 챔피언 5개, 역대 최고 티어, 주 라인 자동 조회
...
### 1. 소환사 추가
- 소환사명을 `닉네임#태그` 또는 `닉네임-태그` 형식으로 입력 (예: `Hide on bush#KR1`)
- op.gg에서 자동 조회:
  - **모스트 챔피언 5개** (역대 기준)
  - **역대 최고 티어** (`GOLD`, `DIAMOND` 등 단순 티어명)
  - **주 라인** (모스트 챔피언 기반 추론)
...
- **모스트 챔피언 5개**: op.gg 메타 정보에서 가장 많이 플레이한 챔피언 5개 추출
- **역대 최고 티어**: 시즌 기록 테이블에서 가장 높은 티어 추출 (`GOLD`, `DIAMOND` 등)
- **주 라인**: 모스트 챔피언 → 챔피언→라인 매핑으로 추론

> 언랭 또는 랭크 기록이 없는 경우 `UNRANKED`로 표시됩니다.
